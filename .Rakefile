...
namespace :supermarket do
  desc 'Publish cookbook to the global chef supermarket'
  task :publish do |t, args|
    cookbooks_directory = args[:cookbook_directory] || ENV['SUPERMARKET_COOKBOOKS_BASE']
    system "cd #{cookbooks_directory} && knife cookbook site share #{readit 'NAME'} Other"
  end
end
namespace :repository do
  ...
  task :git_commit_and_push do
    ...
    git add -f *
    git add ./*
    git commit -a -m "commit for version #{readit 'VERSION'}"
    git tag -a #{::File.read('VERSION').strip} -m "version release #{readit 'VERSION'}"
    git push origin #{`git rev-parse --abbrev-ref HEAD`}
    git push origin #{readit 'VERSION'}
    git commit -a -m "commit for version #{readit 'VERSION'}"
    EOH
    system "#{commit}"
  end
  task :up_minor_version do
    stripped = readit 'VERSION'
...
platforms:
  ...
  task :reconverge => [:clean_dir, :kitchen_yml_workaround, :sync_berkshelf, :destroy, :converge]
